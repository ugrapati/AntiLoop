...
namespace System.Windows.Forms
{
  ...
  //     to start and stop an application, to process Windows messages, and properties
  //     to get information about an application. This class cannot be inherited.
  public sealed class Application
  {
    //
    // Summary:
    //     Represents a method that will check whether the hosting environment is still
    //     sending messages.
    //
    // Returns:
    ...
    private class ClickOnceUtility
    {
      ...
      public static HostType GetHostTypeFromMetaData(string appFullName)
      {
        ...
        try
        {
          System.Deployment.Internal.Isolation.IDefinitionAppId appId = System.Deployment.Internal.Isolation.IsolationInterop.AppIdAuthority.TextToDefinition(0u,appFullName);
    ...
    private class ComponentManager:UnsafeNativeMethods.IMsoComponentManager
    {
      ...
      {
        ComponentHashtableEntry componentHashtableEntry = (ComponentHashtableEntry)OleComponents[dwComponentID];
      ...
      bool UnsafeNativeMethods.IMsoComponentManager.FPushMessageLoop(int dwComponentID,int reason,int pvLoopData)
      {
        ...
        {
          NativeMethods.MSG msg = default(NativeMethods.MSG);
          NativeMethods.MSG[] array = new NativeMethods.MSG[1] { msg };
          bool flag2 = false;
          ComponentHashtableEntry componentHashtableEntry = (ComponentHashtableEntry)OleComponents[dwComponentID];
          if(componentHashtableEntry==null)
          {
    ...
    internal sealed class ThreadContext:MarshalByRefObject, UnsafeNativeMethods.IMsoComponent
    {
          ...
          if(componentManager==null)
          {
            ...
            try
            {
              ...
              if(msoComponentManager!=null&&componentID==-1)
              {
                ...
                if(msoComponentManager.FRegisterComponent(this,mSOCRINFOSTRUCT,out componentID)&&!(msoComponentManager is ComponentManager))
                {
                  messageLoopCount++;
                }
      ...
      internal void BeginModalMessageLoop(ApplicationContext context)
      {
        ...
        try
        {
          ComponentManager?.OnComponentEnterState(componentID,1,0,0,0,0);
        }
        finally
        {
          ourModalLoop=flag;
      ...
      private void DisposeParkingWindow()
      {
        ...
        {
          int lpdwProcessId;
          int windowThreadProcessId = SafeNativeMethods.GetWindowThreadProcessId(new HandleRef(parkingWindow,parkingWindow.Handle),out lpdwProcessId);
      ...
      internal void EndModalMessageLoop(ApplicationContext context)
      {
        ...
        try
        {
          ComponentManager?.FOnComponentExitState(componentID,1,0,0,0);
        }
        finally
        {
          ourModalLoop=flag;
      ...
      internal void FormActivated(bool activate)
      {
        if(activate)
        {
          ...
          {
            msoComponentManager.FOnComponentActivate(componentID);
          }
        }
      }
      ...
      internal void TrackInput(bool track)
      {
        if(track!=GetState(32))
        {
          ...
          if(msoComponentManager!=null&&!(msoComponentManager is ComponentManager))
          {
            msoComponentManager.FSetTrackingComponent(componentID,track);
            SetState(32,track);
          }
        }
      }
      ...
      private void RunMessageLoopInner(int reason,ApplicationContext context)
      {
        ...
        try
        {
          ...
          {
            ComponentManager.FPushMessageLoop(componentID,reason,0);
          }
          ...
          {
            LocalModalMessageLoop(currentForm);
          }
        }
        finally
        {
      ...
      internal bool ProcessFilters(ref NativeMethods.MSG msg,out bool modified)
      {
        bool result = false;
        modified=false;
        if(messageFilters!=null&&!GetState(16))
        {
      ...
      bool UnsafeNativeMethods.IMsoComponent.FContinueMessageLoop(int reason,int pvLoopData,NativeMethods.MSG[] msgPeeked)
      {
        ...
        else
        {
          switch(reason)
          {
            case 1:
            {
              SafeNativeMethods.GetWindowThreadProcessId(new HandleRef(null,UnsafeNativeMethods.GetActiveWindow()),out var lpdwProcessId);
    ...
    internal sealed class ParkingWindow:ContainerControl, IArrangedElement, IComponent, IDisposable
    {
      ...
      internal override void RemoveReflectChild()
      {
        ...
        if(childCount==0&&base.IsHandleCreated)
        {
          ...
          if(threadContext==null||!object.ReferenceEquals(threadContext,ThreadContext.FromCurrent()))
          {
    ...
    internal static bool ComCtlSupportsVisualStyles
    {
      get
      {
        ...
        {
          moduleHandle=UnsafeNativeMethods.LoadLibrary("comctl32.dll");
          if(moduleHandle!=IntPtr.Zero)
          {
            try
            {
            ...
            {
              UnsafeNativeMethods.FreeLibrary(new HandleRef(null,moduleHandle));
            }
          }
        }
    ...
    internal static string CommonAppDataRegistryKeyName
    {
      get
      {
        ...
        return string.Format(CultureInfo.CurrentCulture,format,CompanyName,ProductName,ProductVersion);
      }
    }
    ...
    public static string StartupPath
    {
      get
      {
        if(startupPath==null)
        {
          StringBuilder stringBuilder = new StringBuilder(260);
          UnsafeNativeMethods.GetModuleFileName(NativeMethods.NullHandleRef,stringBuilder,stringBuilder.Capacity);
          startupPath=Path.GetDirectoryName(stringBuilder.ToString());
        }
    ...
    public static RegistryKey UserAppDataRegistry
    {
      ...
      {
        string format = "Software\\{0}\\{1}\\{2}";
        return Registry.CurrentUser.CreateSubKey(string.Format(CultureInfo.CurrentCulture,format,CompanyName,ProductName,ProductVersion));
      }
    }
    ...
    public static event EventHandler Idle
    {
      add
      {
        ...
        lock(threadContext)
        {
          ...
          _=threadContext.ComponentManager;
        }
      }
      remove
      {
    ...
    private static bool SendThemeChanged(IntPtr handle,IntPtr extraParameter)
    {
      ...
      SafeNativeMethods.GetWindowThreadProcessId(new HandleRef(null,handle),out var lpdwProcessId);
    ...
    {
      GetContextForHandle(handle)?.ParkingWindow.ParkHandle(handle);
    }
    ...
    {
      GetContextForHandle(handle)?.ParkingWindow.UnparkHandle(handle);
    }
